<template>
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-4 rounded-xl shadow-xs border border-slate-200/60">
            ...
            <div class="flex flex-col sm:flex-row flex-1 max-w-2xl gap-3">
                ...
                <select
                    ...
                    @change="fetchMembers"
                    class="px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-hidden focus:border-emerald-600 focus:bg-white text-slate-700"
                >
                    <option value="">All Project Assignments</option>
            ...
            >
                ...
                <span>Add Project Proponent</span>
            </button>
        </div>
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-24 bg-white rounded-xl shadow-xs border border-slate-100">
            ...
            <div class="text-slate-500 font-semibold text-sm tracking-wide">Retrieving team rosters...</div>
        </div>
        ...
        <div v-else class="bg-white rounded-xl shadow-xs border border-slate-200/60 overflow-hidden">
            <div v-if="members.length === 0" class="flex flex-col items-center justify-center py-20 text-slate-400">
                ...
                <span class="text-sm font-medium">No personnel records found matching criteria.</span>
            </div>
            ...
            <div v-else class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600 border-collapse">
                    ...
                    <tr>
                        <th class="py-3.5 px-6">Member Profile Details</th>
                        <th class="py-3.5 px-6">Assigned Roster Role</th>
                        <th class="py-3.5 px-6">Assigned Institutional Project</th>
                    ...
                    <tr v-for="member in members" :key="member.id" class="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td class="py-4 px-6">
                            <span :class="member.role === 'leader'
                                ? 'bg-amber-50 text-amber-800 border-amber-100'
                                : 'bg-emerald-50 text-emerald-800 border-emerald-100'"
                                  class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize"
                            >
                                {{ member.role === 'leader' ? 'Project Leader/Chairperson' : 'Team Member' }}
                            </span>
                        </td>
                        <td class="py-4 px-6 max-w-xs">
                            <div class="font-semibold text-slate-700 truncate" :title="member.project?.title">
                                {{ member.project?.title || 'Unassigned Project Core' }}
                            </div>
                        </td>
            ...
            <div class="bg-white rounded-xl shadow-xl border border-slate-200 max-w-lg w-full flex flex-col overflow-hidden animate-modal-pop">
                ...
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 class="text-base font-black text-slate-800">
                        {{ isEditMode ? 'Modify Member Profile Configuration' : 'Register New Project Member' }}
                    </h3>
                ...
                <form @submit.prevent="saveMember" class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Assigned Project Assignment</label>
                        <select
                            v-model="form.project_id"
                            required
                            class="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-hidden focus:border-emerald-600 focus:bg-white text-slate-800"
                        >
                            <option value="" disabled selected>Select an institutional project line</option>
                            <option v-for="project in projectOptions" :key="project.id" :value="project.id">
                                {{ project.title }}
                            </option>
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Roster Focus Role</label>
                        <select
                            v-model="form.role"
                            required
                            class="w-full px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-hidden focus:border-emerald-600 focus:bg-white text-slate-800"
                        >
                            <option value="leader">Project Leader / Chairperson</option>
...
const openAddModal = () => {
    ...
    errorMessage.value = '';
    form.value = { project_id: '', name: '', email: '', role: 'member' };
