        ...
        <div v-else class="space-y-6 animate-fade-in">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-white p-6 rounded-xl shadow-xs border border-slate-200/60 relative overflow-hidden group">
                    ...
                    <div class="mt-2 text-4xl font-black text-slate-800 tracking-tight">{{ stats.total_users ?? 0 }}</div>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-xs border border-slate-200/60 relative overflow-hidden group">
                    ...
                    <div class="mt-2 text-4xl font-black text-slate-800 tracking-tight">{{ stats.total_projects ?? 0 }}</div>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-xs border border-slate-200/60 relative overflow-hidden group">
                    ...
                    <div class="mt-2 text-4xl font-black text-slate-800 tracking-tight">{{ stats.completion_rate ?? 0 }}%</div>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-xs border border-slate-200/60 relative overflow-hidden group">
                    ...
                    <div class="mt-2 text-4xl font-black text-slate-800 tracking-tight">{{ stats.avg_score ?? '0.00' }}</div>
                </div>
            </div>
                    ...
                    <ul class="divide-y divide-slate-100 max-h-[300px] overflow-y-auto pr-1">
                        <li v-for="user in stats.evaluator_workload" :key="user.id" class="py-3 flex justify-between items-center">
                            ...
                            <div class="flex space-x-2">
                                <span class="bg-slate-100 text-slate-600 text-xs px-2 py-0.5 rounded">Assigned: {{ user.assigned_count }}</span>
                                <span class="bg-emerald-100 text-emerald-700 text-xs px-2 py-0.5 rounded font-bold">Done: {{ user.completed_count }}</span>
                            </div>
                        </li>
                    </ul>
                </div>
...
const statusChartData = computed(() => ({
    ...
    datasets: [{
        backgroundColor: ['#047857', '#F59E0B', '#64748B'],
        data: [
            stats.value.status_data?.completed ?? 0,
            stats.value.status_data?.pending ?? 0,
            stats.value.status_data?.draft ?? 0
        ]
    }]
}));
...
const fetchDashboardData = async () => {
    try {
        ...
        if (!user) {
            console.error("No valid user profile found in localStorage.");
            return;
        }
